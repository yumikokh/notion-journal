# Nikki — Notion連携ジャーナル＆AIふりかえりアプリ
（コード名: notion-journal）
...
| ローカル保存 | expo-sqlite（AI会話履歴）/ AsyncStorage（設定）/ expo-secure-store（トークン） |
...
### Epic 4: AIふりかえり（課金ポイント）

| ID | ストーリー | 優先度 |
|----|-----------|--------|
| U-30 | ユーザーとして、今日の記録をもとにAIとチャット形式でふりかえりができる | Must |
| U-31 | ユーザーとして、今週のジャーナルをもとにAIに週次ふりかえりの壁打ちができる | Must |
| U-32 | ユーザーとして、AIが過去の記録から感情・行動パターンを分析してくれる | Should |
| U-33 | ユーザーとして、AIとの会話から生まれた気づきをKPTに直接追加できる | Should |
| U-34 | ユーザーとして、AIに特定のテーマ（仕事、生活、感情）に絞った分析を依頼できる | Could |
| U-35 | ユーザーとして、AIに「先月の自分と今の自分の変化」を聞ける | Could |
| U-36 | ユーザーとして、ふりかえりの会話ログを保存・再閲覧できる | Should |
...
- **日次送り閲覧**: 詳細表示中に左右スワイプで前日/翌日にページめくり遷移。過去の記録を連続して読み返せることが重要

### 6.3 Reflect画面（AIふりかえり）
- チャットUI
- プリセットプロンプト:
  - 「今日を振り返って」
  - 「今週のパターンを教えて」
  ...
  - 「KPTを一緒に考えて」
  - 「先月と比べてどう変わった？」
- AIは直近〜指定範囲のジャーナルデータをコンテキストとして使用
- 会話から「KPTに追加」「Nextに追加」などのアクションボタン
...
> 端末は TanStack Query の永続キャッシュ + expo-sqlite（AI会話履歴）+ AsyncStorage（設定）のみ。
...
CREATE TABLE reflect_messages (
  id TEXT PRIMARY KEY,
  session_id TEXT REFERENCES reflect_sessions(id),
...
```
[System]
あなたはユーザーの内省を支援するコーチです。
ユーザーのジャーナルデータを参照し、パターンの発見や深掘りの質問を通じて
ふりかえりを支援してください。
...
| プロンプト | コンテキスト範囲 | 用途 |
|-----------|-----------------|------|
| 今日を振り返って | 当日 | デイリーふりかえり |
| 今週のパターンを教えて | 直近7日 | 週次分析 |
| 充電/放電を整理して | 直近7日 | ウィークリージャーナル支援 |
| KPTを一緒に考えて | 直近7日 + 先週KPT | KPT作成支援 |
| 先月と比べてどう変わった？ | 直近30日 | 月次ふりかえり |
| 自由に聞く | ユーザー指定 | カスタム |

### 8.3 AIアクション

AIの回答からワンタップでジャーナルに反映できるアクション:
- **KPTに追加**: AIが提案した気づきをKeep/Problem/Tryとして保存
- **Nextに追加**: 次のアクションをデイリージャーナルのNextフィールドに追記
- **ブックマーク**: 会話の特定の発言を保存
...
| **Pro** | AIふりかえり無制限、月次レポート、ワードクラウド、会話ログ保存、KPT自動提案 | ¥580〜980/月 or ¥4,800〜7,800/年 |
...
| **Phase 2** | 2週間 | 閲覧（カレンダー・日次送り）、AIふりかえり、リマインダー |
